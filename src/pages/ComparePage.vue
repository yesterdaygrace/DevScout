...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">Compare Developers</h1>
      <button
    ...
    <div v-if="compareStore.usernames.length === 0" class="text-center py-12 text-gray-400">
      No developers selected for comparison.
      <router-link to="/search" class="text-indigo-500 hover:underline block mt-2">Search developers</router-link>
    </div>

    <div v-else-if="loading" class="text-center py-12 text-gray-500">
      <div class="animate-pulse grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        <div v-for="n in 3" :key="n" class="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-4">
          <div class="flex items-center gap-3">
            ...
            <div class="space-y-2 flex-1">
              ...
              <div class="h-3 w-1/3 bg-gray-200 dark:bg-gray-700 rounded animate-pulse" aria-hidden="true" />
            </div>
          </div>
        </div>
      </div>
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="font-semibold">{{ profile.user.name || profile.user.login }}</p>
                <p class="text-sm text-gray-500">@{{ profile.user.login }}</p>
              </div>
            </div>
            <button @click="compareStore.remove(profile.user.login); profiles = profiles.filter(p => p.user.login !== profile.user.login)">
              <XMarkIcon class="w-5 h-5 text-gray-400 hover:text-red-500" />
            </button>
          </div>
            ...
            <div class="flex flex-col items-center">
              <div class="relative w-20 h-20">
                <svg class="transform -rotate-90 w-20 h-20" aria-hidden="true">
                  <circle cx="40" cy="40" r="32" stroke="currentColor" stroke-width="6" fill="none" class="text-gray-200 dark:bg-gray-700" />
                  <circle
                    cx="40" cy="40" r="32"
          ...
          <div class="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
            <div class="p-2 bg-gray-50 dark:bg-gray-700 rounded">
              <p class="font-bold">{{ profile.user.public_repos }}</p>
              <p class="text-xs text-gray-500">Repos</p>
            </div>
            <div class="p-2 bg-gray-50 dark:bg-gray-700 rounded">
              <p class="font-bold">{{ profile.user.followers }}</p>
              <p class="text-xs text-gray-500">Followers</p>
            </div>
            <div class="p-2 bg-gray-50 dark:bg-gray-700 rounded">
              <p class="font-bold">{{ totalStarsList[idx]?.toLocaleString() || 0 }}</p>
          ...
          <div v-if="profile.skills.length > 0" class="mt-4">
            <p class="text-sm font-medium mb-2 flex items-center gap-1">
              <CodeBracketIcon class="w-4 h-4 text-indigo-500" />
              Skills
            </p>
            <div class="space-y-1.5">
              <div v-for="skill in profile.skills.slice(0, 5)" :key="skill.language" class="flex items-center gap-2">
                <span class="text-xs w-20 truncate">{{ skill.language }}</span>
                <div class="flex-1 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div class="h-full rounded-full bg-indigo-500" :style="{ width: skill.percentage + '%' }" />
                </div>
          ...
          <div v-if="topLanguages[idx]?.length" class="mt-3">
            <p class="text-xs font-medium text-gray-500 mb-1.5">Languages</p>
            <div class="flex flex-wrap gap-1">
              <span v-for="lang in topLanguages[idx]" :key="lang" class="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-xs rounded-full">
                {{ lang }}
              </span>
            </div>
          </div>
        ...
        <div class="overflow-x-auto">
            ...
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700">
                ...
                <th v-for="profile in profiles" :key="profile.user.login" class="text-center py-2 px-3 font-medium">
                  {{ profile.user.login }}
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="metric in [
                ...
                <td class="py-2 pr-4 text-gray-600">{{ metric.label }}</td>
                <td v-for="(_, idx) in profiles" :key="idx" class="text-center py-2 px-3 relative">
                  <span class="font-semibold">{{ metric.format(metric.values[idx]) }}</span>
                  <TrophyIcon v-if="getWinnerIdxs(metric.values, metric.lowerBetter).includes(idx) && profiles.length > 1"
                    class="w-4 h-4 text-yellow-500 inline-block ml-1 -mt-0.5"
                    title="Leader"
                  />
                </td>
              </tr>
              <tr>
                <td class="py-2 pr-4 text-gray-600">Top Languages</td>
                <td v-for="(langs, idx) in topLanguages" :key="idx" class="text-center py-2 px-3">
                  <div class="flex flex-wrap justify-center gap-1">
                    <span v-for="lang in langs" :key="lang" class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-xs rounded">
                      {{ lang }}
                    </span>
                  </div>
                </td>
              ...
              <tr>
                <td class="py-2 pr-4 text-gray-600">Strengths</td>
                <td v-for="(score, idx) in scoreResults" :key="idx" class="text-center py-2 px-3">
                  <div v-if="score.strengths.length > 0" class="flex flex-wrap justify-center gap-1">
                    <span v-for="s in score.strengths" :key="s" class="px-1.5 py-0.5 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 text-xs rounded">
                      {{ s }}
                    </span>
                  </div>
